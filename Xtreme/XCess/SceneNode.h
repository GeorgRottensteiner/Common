...
class CSceneNode : public XOrientation
{
  ...
  public:
    ...
    void QueueRenderAbles( CRenderManager& RManager, XFrustum& Frustum )
    {
      tListObjects::iterator    it( m_listObjects.begin() );
      while ( it != m_listObjects.end() )
      {
    ...
    void PreRender( XRenderer& Renderer )
    {
      ...
      math::matrix4    d3dxmatTemp;
      memcpy( &d3dxmatTemp, &matTemp, sizeof( math::matrix4 ) );

      Renderer.SetTransform( XRenderer::TT_WORLD, d3dxmatTemp );
    }
    ...
    virtual bool          IsVisible( const XFrustum& Frustum )
    {
