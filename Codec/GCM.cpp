...
static void gcm_mult( gcm_context* ctx,     // pointer to established context
  const GR::u8 x[16],    // pointer to 128-bit input vector
...
int gcm_setkey( gcm_context* ctx,   // pointer to caller-provided gcm context
  const GR::u8* key,   // pointer to the AES encryption key
          ...
          // 128, 192 or 256-bit keys respectively)
{
  ...
  ctx->len = 0;
  ctx->add_len = 0;
  memset( ctx->HL, 0, sizeof( ctx->HL ) );
  memset( ctx->HH, 0, sizeof( ctx->HH ) );
  memset( ctx->base_ectr, 0, sizeof( ctx->base_ectr ) );
  ...
  if ( !ctx->aes_ctx.Initialise( true, ByteBuffer( key, keysize ), ByteBuffer() ) )
  {
    return 1;
  }
  ...
  if ( !ctx->aes_ctx.TransformBlock( data, data ) )
  {
    return 1;
  }
  ...
  size_t add_len )     // length of additional AEAD data (bytes)
{
  ...
  ByteBuffer  data( ctx->y, 16 );
  ByteBuffer  out( 16 );
  if ( !ctx->aes_ctx.TransformBlock( data, out ) )
  {
    return 1;
  }
  memcpy( ctx->base_ectr, out.Data(), sizeof( ctx->base_ectr ) );
...
int gcm_finish( gcm_context* ctx,   // pointer to user-provided GCM context
  GR::u8* tag,         // pointer to buffer which receives the tag
  ...
  size_t tag_len )        // byte length of the tag <= 16
{
  ...
  */
  gcm_crypt_and_tag( ctx, GCM_DECRYPT, iv, iv_len, add, add_len,
...
int aes_gcm_encrypt( unsigned char* output, const unsigned char* input, int input_length, const unsigned char* key, const size_t key_len, const unsigned char* iv, const size_t iv_len )
{
  ...
  gcm_setkey( &ctx, key, (const GR::u32)key_len );

  ret = gcm_crypt_and_tag( &ctx, GCM_ENCRYPT, iv, iv_len, NULL, 0,
...
int aes_gcm_decrypt( unsigned char* output, const unsigned char* input, int input_length, const unsigned char* key, const size_t key_len, const unsigned char* iv, const size_t iv_len )
{
  ...
  ret = gcm_crypt_and_tag( &ctx, GCM_DECRYPT, iv, iv_len, NULL, 0,
    input, output, input_length, tag_buf, tag_len );
