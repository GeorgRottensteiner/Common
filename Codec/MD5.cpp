...
void MD5::finalize ()
{
  ...
  if (finalized)
  {
    dh::Log( "MD5::finalize:  Already finalized this digest!\n" );
    return;
  }
...
ByteBuffer MD5::raw_digest()
{
  ...
  memcpy( (unsigned char*)digestBuffer.Data(), digest, 16 );
  return digestBuffer;
}



GR::String MD5::hex_digest()
{
  int i;
  char  s[33];
  ...
  if ( !finalized )
  {
    dh::Log( "MD5::hex_digest:  Can't get digest if you haven't finalized the digest!\n" );
    return "";
  }
...
bool MD5::CalcHash( const ByteBuffer& Data, ByteBuffer& Hash )
{
  MD5    Instance;

  Instance.init();
  Instance.update( (GR::u8*)Data.Data(), (GR::u32)Data.Size() );
  Instance.finalize();

  Hash.Resize( 16 );
  ::memcpy( (void*)Hash.Data(), Instance.digest, 16 );
  return true;
}



bool MD5::CalcKeyedHash( const ByteBuffer& Data,
