...
namespace Codec
{
  ...
  int Rijndael::init(Mode mode,Direction dir,const GR::u8 * key,KeyLength keyLen,GR::u8 * initVector)
  {
    // Not initialized yet
    m_state = Invalid;

    // Check the mode
    if((mode != CBC) && (mode != ECB) && (mode != CFB1))return RES_UNSUPPORTED_MODE;
    m_mode = mode;

    // And the direction
    if((dir != Encrypt) && (dir != Decrypt))return RES_UNSUPPORTED_DIRECTION;
    m_direction = dir;
    ...
    if(!key)return RES_BAD_KEY;

    GR::u8 keyMatrix[_MAX_KEY_COLUMNS][4];

    for(GR::u32 i = 0;i < uKeyLenInBytes;i++)
  ...
  int Rijndael::blockEncrypt(const GR::u8 *input,int inputLen,GR::u8 *outBuffer)
  {
    int i, k, numBlocks;
    GR::u8 block[16], iv[4][4];

    if(m_state != Valid)return RES_NOT_INITIALIZED;
    if(m_direction != Encrypt)return RES_BAD_DIRECTION;
    ...
    switch(m_mode){
      ...
      break;
      case CBC:
        ((GR::u32*)block)[0] = ((GR::u32*)m_initVector)[0] ^ ((GR::u32*)input)[0];
        ((GR::u32*)block)[1] = ((GR::u32*)m_initVector)[1] ^ ((GR::u32*)input)[1];
        ((GR::u32*)block)[2] = ((GR::u32*)m_initVector)[2] ^ ((GR::u32*)input)[2];
        ((GR::u32*)block)[3] = ((GR::u32*)m_initVector)[3] ^ ((GR::u32*)input)[3];
        encrypt(block,outBuffer);
        input += 16;
        for(i = numBlocks - 1;i > 0;i--)
        {
          ((GR::u32*)block)[0] = ((GR::u32*)outBuffer)[0] ^ ((GR::u32*)input)[0];
          ((GR::u32*)block)[1] = ((GR::u32*)outBuffer)[1] ^ ((GR::u32*)input)[1];
          ((GR::u32*)block)[2] = ((GR::u32*)outBuffer)[2] ^ ((GR::u32*)input)[2];
          ((GR::u32*)block)[3] = ((GR::u32*)outBuffer)[3] ^ ((GR::u32*)input)[3];
          outBuffer += 16;
          encrypt(block,outBuffer);
          input += 16;
        }
        ...
        for(i = numBlocks; i > 0; i--)
        {
          for(k = 0; k < 128; k++)
          {
            ...
            *((GR::u32*)(block+12)) = *((GR::u32*)iv[3]);
            ...
            iv[3][3] = (GR::u8)( (iv[3][3] << 1) | (outBuffer[k/8] >> (7-(k&7))) & 1 );
          }
        }
      break;
      default:
  ...
  int Rijndael::padEncrypt(const GR::u8 *input, int inputOctets, GR::u8 *outBuffer)
  {
    int i, numBlocks, padLen;
    GR::u8 block[16], *iv;

    if(m_state != Valid)return RES_NOT_INITIALIZED;
    if(m_direction != Encrypt)return RES_NOT_INITIALIZED;
  ...
  //      assert(padLen > 0 && padLen <= 16);
      ...
      case CBC:
        ...
        for(i = numBlocks; i > 0; i--)
        {
          ((GR::u32*)block)[0] = ((GR::u32*)input)[0] ^ ((GR::u32*)iv)[0];
          ((GR::u32*)block)[1] = ((GR::u32*)input)[1] ^ ((GR::u32*)iv)[1];
          ((GR::u32*)block)[2] = ((GR::u32*)input)[2] ^ ((GR::u32*)iv)[2];
          ((GR::u32*)block)[3] = ((GR::u32*)input)[3] ^ ((GR::u32*)iv)[3];
          encrypt(block, outBuffer);
          iv = outBuffer;
          input += 16;
          outBuffer += 16;
  ...
  int Rijndael::blockDecrypt(const GR::u8 *input, int inputLen, GR::u8 *outBuffer)
  {
    int i, k, numBlocks;
    GR::u8 block[16], iv[4][4];

    if(m_state != Valid)return RES_NOT_INITIALIZED;
    if((m_mode != CFB1) && (m_direction == Encrypt))return RES_BAD_DIRECTION;
  ...
  #else
          *((GR::u32*)iv[0]) = ((GR::u32*)input)[0]; ((GR::u32*)outBuffer)[0] = ((GR::u32*)block)[0];
          *((GR::u32*)iv[1]) = ((GR::u32*)input)[1]; ((GR::u32*)outBuffer)[1] = ((GR::u32*)block)[1];
          *((GR::u32*)iv[2]) = ((GR::u32*)input)[2]; ((GR::u32*)outBuffer)[2] = ((GR::u32*)block)[2];
          *((GR::u32*)iv[3]) = ((GR::u32*)input)[3]; ((GR::u32*)outBuffer)[3] = ((GR::u32*)block)[3];
  #endif
          input += 16;
          outBuffer += 16;
        }
        ...
        for(i = numBlocks; i > 0; i--)
        {
          for(k = 0; k < 128; k++)
          {
            ...
            *((GR::u32*)(block+12)) = *((GR::u32*)iv[3]);
            ...
            iv[3][1] = (GR::u8)( (iv[3][1] << 1) | (iv[3][2] >> 7) );
            iv[3][2] = (GR::u8)( (iv[3][2] << 1) | (iv[3][3] >> 7) );
            iv[3][3] = (GR::u8)( (iv[3][3] << 1) | (input[k/8] >> (7-(k&7))) & 1 );
            outBuffer[k/8] ^= (block[0] & 0x80) >> (k & 7);
          }
        }
      break;
  ...
  int Rijndael::padDecrypt(const GR::u8 *input, int inputOctets, GR::u8 *outBuffer)
  {
    int i, numBlocks, padLen;
    GR::u8 block[16];
    GR::u32 iv[4];
    ...
    if(m_state != Valid)return RES_NOT_INITIALIZED;
    if(m_direction != Decrypt)return RES_BAD_DIRECTION;

    if(input == 0 || inputOctets <= 0)return 0;

    if((inputOctets % 16) != 0)return RES_CORRUPTED_DATA;
    ...
    switch(m_mode){
      case ECB:
        ...
        decrypt(input, block);
        padLen = block[15];
        if (padLen >= 16)return RES_CORRUPTED_DATA;
        for(i = 16 - padLen; i < 16; i++)
        {
          if(block[i] != padLen)return RES_CORRUPTED_DATA;
        }
      ...
      case CBC:
        ...
        ((GR::u32*)block)[3] ^= iv[3];
        padLen = block[15];
        if(padLen <= 0 || padLen > 16)return RES_CORRUPTED_DATA;
        for(i = 16 - padLen; i < 16; i++)
        {
          if(block[i] != padLen)return RES_CORRUPTED_DATA;
        }
  ...
  void Rijndael::keySched(GR::u8 key[_MAX_KEY_COLUMNS][4])
  {
    int j,rconpointer = 0;

    // Calculate the necessary round keys
    // The number of calculations depends on keyBits and blockBits
    int uKeyColumns = (int)m_uRounds - 6;

    GR::u8 tempKey[_MAX_KEY_COLUMNS][4];
  ...
  void Rijndael::keyEncToDec()
  {
    ...
    {
      w = m_expandedKey[r][0];
      *((GR::u32*)w) = *((GR::u32*)U1[w[0]]) ^ *((GR::u32*)U2[w[1]]) ^ *((GR::u32*)U3[w[2]]) ^ *((GR::u32*)U4[w[3]]);
      w = m_expandedKey[r][1];
      *((GR::u32*)w) = *((GR::u32*)U1[w[0]]) ^ *((GR::u32*)U2[w[1]]) ^ *((GR::u32*)U3[w[2]]) ^ *((GR::u32*)U4[w[3]]);
      w = m_expandedKey[r][2];
      *((GR::u32*)w) = *((GR::u32*)U1[w[0]]) ^ *((GR::u32*)U2[w[1]]) ^ *((GR::u32*)U3[w[2]]) ^ *((GR::u32*)U4[w[3]]);
  ...
  void Rijndael::encrypt(const GR::u8 a[16], GR::u8 b[16])
  {
    ...
    *((GR::u32*)temp[0]) = *((GR::u32*)(b   )) ^ *((GR::u32*)m_expandedKey[m_uRounds-1][0]);
    *((GR::u32*)temp[1]) = *((GR::u32*)(b+ 4)) ^ *((GR::u32*)m_expandedKey[m_uRounds-1][1]);
    *((GR::u32*)temp[2]) = *((GR::u32*)(b+ 8)) ^ *((GR::u32*)m_expandedKey[m_uRounds-1][2]);
    *((GR::u32*)temp[3]) = *((GR::u32*)(b+12)) ^ *((GR::u32*)m_expandedKey[m_uRounds-1][3]);
    b[ 0] = T1[temp[0][0]][1];
    b[ 1] = T1[temp[1][1]][1];
    b[ 2] = T1[temp[2][2]][1];
    b[ 3] = T1[temp[3][3]][1];
    ...
    b[14] = T1[temp[1][2]][1];
    b[15] = T1[temp[2][3]][1];
    *((GR::u32*)(b   )) ^= *((GR::u32*)m_expandedKey[m_uRounds][0]);
    *((GR::u32*)(b+ 4)) ^= *((GR::u32*)m_expandedKey[m_uRounds][1]);
    *((GR::u32*)(b+ 8)) ^= *((GR::u32*)m_expandedKey[m_uRounds][2]);
    *((GR::u32*)(b+12)) ^= *((GR::u32*)m_expandedKey[m_uRounds][3]);
  }
  ...
  void Rijndael::decrypt(const GR::u8 a[16], GR::u8 b[16])
  {
    ...
    GR::u8 temp[4][4];
      ...
      *((GR::u32*)temp[0]) = *((GR::u32*)(a   )) ^ *((GR::u32*)m_expandedKey[m_uRounds][0]);
      *((GR::u32*)temp[1]) = *((GR::u32*)(a+ 4)) ^ *((GR::u32*)m_expandedKey[m_uRounds][1]);
      *((GR::u32*)temp[2]) = *((GR::u32*)(a+ 8)) ^ *((GR::u32*)m_expandedKey[m_uRounds][2]);
      *((GR::u32*)temp[3]) = *((GR::u32*)(a+12)) ^ *((GR::u32*)m_expandedKey[m_uRounds][3]);
  ...
  bool Rijndael::EncryptCBC256( const ByteBuffer& In, const ByteBuffer& Key, ByteBuffer& Encrypted, const ByteBuffer& IV )
  {
    if ( Key.Size() != 32 )
    {
      return false;
    ...
    Encrypted.Resize( In.Size() + 16 );

    Rijndael      Rin;

    Rin.init( Rijndael::CBC, Rijndael::Encrypt, (const GR::u8*)Key.Data(), Rijndael::Key32Bytes );
  ...
  bool Rijndael::DecryptCBC256( const ByteBuffer& In, const ByteBuffer& Key, ByteBuffer& Decrypted, const ByteBuffer& IV )
  {
    if ( Key.Size() != 32 )
    {
      return false;
    ...
    Decrypted.Resize( In.Size() + 16 );

    Rijndael      Rin;

    Rin.init( Rijndael::CBC, Rijndael::Decrypt, (const GR::u8*)Key.Data(), Rijndael::Key32Bytes );

    int len = Rin.padDecrypt( (const GR::u8*)In.Data(), (int)In.Size(), (GR::u8*)Decrypted.Data() );
